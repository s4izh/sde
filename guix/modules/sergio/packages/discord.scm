...
(define discord-disable-breaking-updates
  (with-extensions (list (@ (gnu packages guile) guile-json-4))
                   (program-file "disable-breaking-updates"
                                 #~(begin
                                     (use-modules (json))
                                     (let* ((config-home (or (getenv
                                                              "XDG_CONFIG_HOME")
                                                             (string-append (getenv
                                                                             "HOME")
                                                              "/.config")))
                                            (settings-path (string-append
                                                            config-home
                                                            "/discord/settings.json"))
                                            (settings-tmp (string-append
                                                           settings-path
                                                           ".tmp"))
                                            (settings (if (file-exists?
                                                          ...
                                                          (with-exception-handler (lambda 
                                                                                    ...
                                                                                    (exit
                                                                                     0))
                                                           (lambda ()
                                                             (with-input-from-file settings-path
                                                               json->scm))
                                                           #:unwind? #t
                                                           #:unwind-for-type 'json-invalid)
                                                          (list)))
                                            (skip (assoc-ref settings
                                                   "SKIP_HOST_UPDATE")))
                                       ;; Enable updates when environment variable is set.
                                       (unless (eq? skip
                                                    (not (getenv
                                                          "DISCORD_ENABLE_UPDATES")))
                                         (set! skip
                                               (not skip))
                                         (with-output-to-file settings-tmp
                                           (lambda ()
                                             (scm->json (assoc-set! settings
                                                         "SKIP_HOST_UPDATE"
                                                         skip))))
                                         (rename-file settings-tmp
                                                      settings-path)
                                         (display (if skip "Disabled updates"
                                                      "Enabled updates"))
                                         (newline)))))))
...
(define-public discord
  (package
    ...
    (arguments
     (list
      #:patchelf-plan #~`(("Discord")
                          ("chrome_crashpad_handler")
                          ("chrome-sandbox"))
      #:install-plan #~`(("." "opt/discord")
